function [omega_new, oct_new, zeta_new] = GBdist(data,pgnum,genplot,waitbarQ,epsijk)
arguments
    data(:,16)
    pgnum(1,1) double
    genplot(1,1) logical = false
    waitbarQ(1,1) logical = false
    epsijk(1,1) double = 1
end
%% INPUT DATA
%
...
%                     end
					...
					qSA = qmult(Si,qA,epsijk);
					qSB = qmult(Sj,qB,epsijk);
					qSC = qmult(Sm,qC,epsijk);
					qSD = qmult(Sl,qD,epsijk);
...
% 					qzm1 = [cos(zm1/2) 0 0 -epsijk*sin(zm1/2)];
					qCz1 = qmult(qSC,qzm1,epsijk);
					qDz1 = qmult(qSD,qzm1,epsijk);
...
% 					qsm1 = [cos(sm1/2) 0 0 -epsijk*sin(sm1/2)];
					qCs1 = qmult(qSC,qsm1,epsijk);
					qDs1 = qmult(qSD,qsm1,epsijk);
...
% 					qzm2 = [cos(zm2/2) 0 0 -epsijk*sin(zm2/2)];
					qCz2 = qmult(qSC,qzm2,epsijk);
					qDz2 = qmult(qSD,qzm2,epsijk);
...
% 					qsm2 = [cos(sm2/2) 0 0 -epsijk*sin(sm2/2)];
					qCs2 = qmult(qSC,qsm2,epsijk);
					qDs2 = qmult(qSD,qsm2,epsijk);
...
crossAC = cross(qA(2:4),qC(2:4));
crossBD = cross(qB(2:4),qD(2:4));
...
qi = pp(1)*q + qq(1)*p + epsijk*cross(p,q);
