function [props,varargout] = interp_bary(mesh,intfacetIDs,qm2,nA2,usv,barytype,barytol,projtol,nnMax,brkQ,NV)
arguments
    mesh(1,1) struct {mustContainFields(mesh,{'pts','ppts','props'})}
    intfacetIDs cell
    qm2(:,4) double
    nA2(:,3) double
    usv(1,1) struct
...
if ~isempty(NV.databary)
    ...
    facetprops = get_facetprops(mesh,intfacetIDs);
    ...
    varargout = {facetprops,NNextrapID,nnList};
    
else
    %% whne barycentric coordinates aren't specified (new predict points)
