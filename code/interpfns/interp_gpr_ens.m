function [ypred,ypredlist] = interp_gpr_ens(gprMdls,qm2,nA2,projtol,usv,zeroQ)
...
nmdls = length(gprMdls);
ypredlist = cell(1,nmdls);
for i = 1:nmdls
    gprMdl = gprMdls{i};
    ypredlist{i} = predict(gprMdl,pts2);
end
ypred = mean([ypredlist{:}],2);
